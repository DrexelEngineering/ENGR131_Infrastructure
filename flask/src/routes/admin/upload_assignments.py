...
def upload_students_():
    ...
    if 'assignment_id' not in request.form:
        ...
        if is_string_number_combo(assignment_id):
            ...
            return 'assignment_id must be a string-number combo in the format AssignmentType_AssignmentNumber', 400
    ...
    if file:
        ...
        assignment_data = [{"assignment_name": assignment_name,
        ...
        for data in json_extracted:
            question_data.append({"question_id": data.get('question_id', ''),
                                "assignment_id": assignment_id,
        ...
        sql_call = upsert_json(question_data, 'questions', ["assignment_id", "question_id"])
        if not commit_sql(cursor, sql_call):
            conn.rollback()  # Rollback and close connection on failure
            conn.close()
            return "Error occurred during SQL execution", 500
